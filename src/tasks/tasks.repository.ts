...
import {
  Usuario,
  UsuarioDocument,
} from 'src/usuarios/interfaces/usuarios.schema';
...
export class TasksRepository {
  ...
  async createTask(createTaskDto: CreateTaskDto): Promise<ITaskEntity> {
    const taskExistente = await this.taskModel.findOne({
    ...
    if (createTaskDto.usuario) {
      ...
      if (usuario.papel === 'admin') {
        throw new BadRequestException(
          'Usuários com papel admin não podem criar tarefas para o mesmo nível de papel',
        );
      }
    }
    ...
    const data = {
      ...createTaskDto,
      usuario: createTaskDto.usuario
        ? new Types.ObjectId(createTaskDto.usuario)
        : undefined,
    };
